...
const MOCK_PERFORMANCE = {
  completedTotal: 38,
  overdueTotal: 3,
  comparisonPercentage: 12.4,
  completedPercentage: 76.5,
};

const MOCK_OVERDUE: OverdueTask[] = [
  { id: '1', title: 'Finalize landing page copy', dueDate: '2026-05-21' },
  { id: '2', title: 'Review API documentation', dueDate: '2026-05-19' },
  { id: '3', title: 'Customer interview notes', dueDate: '2026-05-15' },
];
...
export default function SelfPerformanceScreen() {
  ...
  const completed = clamp(MOCK_PERFORMANCE.completedPercentage);
      ...
      <View className="mb-6 flex-row flex-wrap gap-3">
        <Card className="min-w-[45%] flex-1 p-4">
          ...
          <Text className="mt-1 text-2xl font-black text-foreground">
            {MOCK_PERFORMANCE.completedTotal}
          </Text>
        </Card>
        <Card className="min-w-[45%] flex-1 p-4">
        ...
        <Card className="min-w-[45%] flex-1 p-4">
          ...
          <Text className="mt-1 text-2xl font-black text-foreground">
            {formatTrend(MOCK_PERFORMANCE.comparisonPercentage)}
          </Text>
        </Card>
      </View>
      ...
      <Card className="mb-6">
        <View className="mb-1 flex-row items-center justify-between">
          ...
          <View className="rounded-full bg-destructive/10 px-3 py-1">
            <Text className="text-xs font-semibold text-destructive">
              {MOCK_PERFORMANCE.overdueTotal} overdue
            </Text>
          </View>
        </View>
        ...
        {MOCK_OVERDUE.length === 0 ? (
          <View className="rounded-md border border-dashed border-border p-4">
            <Text className="text-sm text-muted-foreground">No overdue tasks in this range.</Text>
          </View>
        ) : (
          <View className="gap-3">
            {MOCK_OVERDUE.map((task) => (
              <View
                key={task.id}
